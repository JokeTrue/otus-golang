...
func TestRun(t *testing.T) {
	...
	t.Run("Every third task produces error: successed", func(t *testing.T) {
		tasksCount := 30
		tasks := make([]Task, 0, tasksCount)
		...
		result := Run(tasks, 10, 11)

		require.Nil(t, result)
		require.Equal(t, int32(tasksCount), runTasksCount, "not all tasks were completed")
	})

	t.Run("Every third task produces error: failed", func(t *testing.T) {
		tasksCount := 30
		tasks := make([]Task, 0, tasksCount)
		...
		result := Run(tasks, 10, 8)
		require.Error(t, ErrErrorsLimitExceeded, result)
	})
}
